...
def _load_env_file_force_app_env() -> Optional[str]:
    try:
        ...
        fixed = Path(r"/APTIV/app/.env")
        env_files.append(fixed)

        here = Path(__file__).resolve()
        app_dir = here.parents[1]
        env_files.append(app_dir / ".env")
...
PLAYWRIGHT_BROWSERS_DIR = _env("PLAYWRIGHT_BROWSERS_DIR", default="").strip()
PLAYWRIGHT_CHROMIUM_EXECUTABLE = _env("PLAYWRIGHT_CHROMIUM_EXECUTABLE", default="").strip()

SNAP_LOCK_STALE_SEC = _env_int("SNAP_LOCK_STALE_SEC", "SNAPSHOT_LOCK_STALE_SEC", default=1800)
...
def _log_boot_config() -> None:
    ...
    _log(f"[BOOT] PLAYWRIGHT_BROWSERS_DIR={PLAYWRIGHT_BROWSERS_DIR or 'EMPTY'}")
    _log(f"[BOOT] PLAYWRIGHT_CHROMIUM_EXECUTABLE={PLAYWRIGHT_CHROMIUM_EXECUTABLE or 'EMPTY'}")
    _log(f"[BOOT] RUN_MAILER_TRIGGER={_env('RUN_MAILER_TRIGGER', default='')}")
    _log(f"[BOOT] SNAP_FORCE_SEND={_env('SNAP_FORCE_SEND', default='')}")
    _log(f"[BOOT] sys.executable={sys.executable}")
    _log(f"[BOOT] frozen={_is_frozen_app()}")
    _log("[BOOT] ==================================")
...
def _candidate_browser_executables() -> List[Path]:
    ...
    if PLAYWRIGHT_CHROMIUM_EXECUTABLE:
        candidates.append(Path(PLAYWRIGHT_CHROMIUM_EXECUTABLE))

    if PLAYWRIGHT_BROWSERS_DIR:
        root = Path(PLAYWRIGHT_BROWSERS_DIR)
        candidates.extend(
            [
                ...
                root / "chromium_headless_shell" / "chrome-headless-shell-win64" / "chrome-headless-shell.exe",
            ]
        )
    ...
    for base in _deploy_root_candidates():
        candidates.extend(
            [
                base / "playwright-browsers" / "chrome-win" / "chrome.exe",
                base / "playwright-browsers" / "chrome-win64" / "chrome.exe",
                base / "playwright-browsers" / "chrome-headless-shell-win64" / "chrome-headless-shell.exe",
                base / "playwright-browsers" / "chromium" / "chrome-win" / "chrome.exe",
                base / "playwright-browsers" / "chromium" / "chrome-win64" / "chrome.exe",
                base / "playwright-browsers" / "chromium_headless_shell" / "chrome-headless-shell-win64" / "chrome-headless-shell.exe",
            ]
        )
...
def _resolve_browser_executable() -> Optional[str]:
    candidates = _candidate_browser_executables()
    for p in candidates:
        try:
            if p.exists() and p.is_file():
                _log(f"[PW] chromium executable resolved={p}")
                return str(p)
        except Exception:
            continue

    if candidates:
        _log("[PW] chromium executable candidates checked:")
        for p in candidates:
            _log(f"[PW]   - {p}")
    else:
        _log("[PW] chromium executable candidates checked: (none)")
    return None
...
def _lock_file_age_sec() -> Optional[float]:
    if not LOCK_FILE:
        return None
    try:
...
def _is_stale_lock(info: Dict[str, Any]) -> Tuple[bool, str]:
    pid = int(info.get("pid", 0) or 0)

    if pid > 0:
        alive = _is_pid_alive(pid)
        if alive:
            return False, f"pid_alive pid={pid}"
        return True, f"pid_dead pid={pid}"

    age = _lock_file_age_sec()
    if age is not None and age >= max(60, SNAP_LOCK_STALE_SEC):
        return True, f"age_exceeded age_sec={age:.1f}"

    return False, "unknown_and_not_old_enough"


def _delete_stale_lock_if_needed() -> bool:
    if not LOCK_FILE:
        return False
    ...
    stale, reason = _is_stale_lock(info)
    _log(f"[LOCK] stale_check stale={stale} reason={reason} info={_short_text(info, 300)}")
    ...
    try:
        os.remove(LOCK_FILE)
        _log(f"[LOCK] stale lock removed LOCK_FILE={LOCK_FILE} reason={reason}")
        return True
    except Exception as e:
        _log(f"[LOCK] stale lock remove fail LOCK_FILE={LOCK_FILE}: {_fmt_exc(e)}")
        return False
...
def _acquire_lock() -> bool:
    ...
    for attempt in range(2):
        try:
            ...
            try:
                payload = json.dumps(_lock_payload(), ensure_ascii=False, indent=2)
            ...
            _log(f"[LOCK] acquire hit existing LOCK_FILE={LOCK_FILE} attempt={attempt+1}")
            removed = _delete_stale_lock_if_needed()
            if removed:
                time.sleep(0.1)
                continue
            return False
...
def _release_lock() -> None:
    ...
    try:
        ...
        if lock_pid not in (0, my_pid):
            _log(f"[LOCK] release skipped not-owner lock_pid={lock_pid} my_pid={my_pid}")
            return
...
def _current_trigger_name() -> str:
    return _env("RUN_MAILER_TRIGGER", default="").strip().lower()


def _is_manual_trigger() -> bool:
    trig = _current_trigger_name()
    if trig in ("manual_stop_mail", "manual", "force_send"):
        return True
    if _env_bool("SNAP_FORCE_SEND", default=False):
        return True
    return False
...
def _is_snap_marker_ready(page) -> bool:
    try:
        return bool(
            page.evaluate(
                """
                () => {
                  const el = document.getElementById("snap-ready");
                  if (!el) return false;
                  return String(el.getAttribute("data-snap-ready") || "0") === "1";
                }
                """
            )
        )
...
def _is_visual_sections_ready(page) -> bool:
    try:
        return bool(
            page.evaluate(
                """
...
def _wait_data_ready(page, timeout_ms: int) -> None:
    ...
    while time.time() < deadline:
        ...
        try:
            hard_ready = page.locator("#__snap_ready__").count() > 0
        except Exception:
            hard_ready = False

        marker_ready = _is_snap_marker_ready(page)
        visual_ready = _is_visual_sections_ready(page)
        ...
        if ready_now:
            if stable_ok_since is None:
                stable_ok_since = time.time()
            elif (time.time() - stable_ok_since) >= stable_need_sec:
                _log(
                    f"[DATA] ready hard_ready={hard_ready} marker_ready={marker_ready} "
                    f"visual_ready={visual_ready} spinner={sc}"
                )
                return
        else:
            stable_ok_since = None
...
def _capture_pages_pdf_bytes(
    ...
    if browser_executable:
        ...
        _log(f"[SNAPSHOT] chromium executable override={browser_executable}")
    else:
        _log("[SNAPSHOT] chromium executable override not found -> fallback to bundled playwright browser")
    ...
    with sync_playwright() as p:
        ...
        _log("[SNAPSHOT] browser context created locale=ko-KR timezone=Asia/Seoul")
        ...
        for i, ps in enumerate(pages):
            ...
            try:
                ...
                _log(f"[SNAPSHOT] no extra click in snap mode page={ps.name}")
                ...
                if TS_OVERLAY_ENABLE and (not overlay_done) and ps.page_id == "02_production_info":
                    ts_txt = anchor_now.astimezone(KST).strftime(TS_OVERLAY_FMT)
...
def run_snapshot_and_mail(
    ...
    trigger_name = _current_trigger_name()
    manual_trigger = _is_manual_trigger()

    _write_status("running", anchor_now=anchor_now.isoformat(), trigger=trigger_name or "none")
    if _DOTENV_LOADED:
    ...
    if not _acquire_lock():
        ...
        _write_status("idle", note="lock_busy", trigger=trigger_name or "none")
        _close_log_file()
        if manual_trigger:
            raise RuntimeError("snapshot_mailer lock busy during manual trigger")
        return []
    ...
    try:
        ...
        _log(f"[SNAPSHOT] run start day={prod_day_n} shift={shift_type_n} mode={SNAP_MODE} trigger={trigger_name or 'none'} pages={[p.name for p in pages]}")

        _post_event_log("start", dict(slot_payload_base))

        key = _sent_key(prod_day_n, shift_type_n)
        if (not FORCE_SEND) and (not manual_trigger) and SKIP_IF_ALREADY_SENT and SENT_FILE:
            last = _read_last_sent()
            if last == key:
                _log(f"[SENT] already sent for {key} (SENT_FILE={SENT_FILE}) -> skip")
                _write_status("idle", note="already_sent", prod_day=prod_day_n, shift_type=shift_type_n, trigger=trigger_name or "none")
                _post_event_log("skip_already_sent", dict(slot_payload_base))
                return []
        else:
            _log(
                f"[SENT] skip check bypass "
                f"FORCE_SEND={FORCE_SEND} manual_trigger={manual_trigger} "
                f"SKIP_IF_ALREADY_SENT={SKIP_IF_ALREADY_SENT}"
            )
        ...
        if SNAP_MODE == "mail" and not recips:
            ...
            _write_status("error", note="recipients=0", prod_day=prod_day_n, shift_type=shift_type_n, trigger=trigger_name or "none")
        ...
        if not attachments:
            ...
            _write_status("error", note="no_pdf", prod_day=prod_day_n, shift_type=shift_type_n, trigger=trigger_name or "none")
        ...
        body_lines = [
            ...
            f"- trigger={trigger_name or 'none'}",
        ...
        if SNAP_MODE == "mail":
            ...
            if mailed:
                ...
                if (not manual_trigger) and SENT_FILE:
                    _write_last_sent(key)
                else:
                    _log("[SENT] manual trigger -> last_sent not written")
            else:
        ...
        if mailed or (SNAP_MODE == "print"):
            ...
            _log(f"[SNAPSHOT] run end mailed={mailed} pdf_count={len(names)} trigger={trigger_name or 'none'}")
            return names
        ...
        _write_status("error", note=str(e)[:200], trigger=trigger_name or "none")
...
def _candidate_run_mailer_paths() -> List[Path]:
    ...
    aptiv_root = _env("APTIV_ROOT", default="").strip()
    if aptiv_root:
        try:
            root = Path(aptiv_root).resolve()
            candidates.extend(
                [
                    root / "run_mailer.exe",
                    root / "run_mailer.dist" / "run_mailer.exe",
                    root / "launcher.dist" / "run_mailer.exe",
                ]
            )
        except Exception:
            pass
    ...
    try:
        ...
        candidates.extend(
            [
                ...
                exe_dir.parent / "launcher.dist" / "run_mailer.exe",
            ]
        )
    except Exception:
        pass
    ...
    try:
        for base in _deploy_root_candidates():
            candidates.extend(
                [
                    base / "run_mailer.exe",
                    base / "run_mailer.dist" / "run_mailer.exe",
                    base / "launcher.dist" / "run_mailer.exe",
                ]
            )
    except Exception:
        pass
...
def _resolve_run_mailer_command(slot_dt: datetime) -> List[str]:
    ...
    if not _is_frozen_app():
        py = _python_cmd()
        cmd = [py, "-m", "app.run_mailer", CLI_RUN_ONCE_FLAG, slot_iso]
    ...
    for p in candidates:
        try:
            if p.exists() and p.is_file():
                ...
                if current_exe and resolved == current_exe:
                    _log(f"[SCHED][SUBPROC] skip self executable candidate={p}")
                    continue
