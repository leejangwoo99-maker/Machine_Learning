...
def _snapshot_requested_scope() -> Tuple[str, str]:
    d = _norm_day(_qp_get("prod_day", ""))
    s = _norm_shift(_qp_get("shift_type", ""))
    if d:
        return d, s
...
def _mount_alarm_sse(now_day: str, now_shift: str, view_day: str, view_shift: str):
    """
    서버 구독은 now_day/now_shift로 유지하되,
...
if "p02_active_day" not in st.session_state or "p02_active_shift" not in st.session_state:
    if SNAP_MODE:
        d0, s0 = _snapshot_requested_scope()
    else:
...
if st.session_state.get("p02_do_full_refresh", False):
    if SNAP_MODE:
        nd, ns = _snapshot_requested_scope()
    else:
...
st.caption(
    f"현재 조회: prod_day={active_day} / shift_type={active_shift} | 모드: 수동(버튼 기반)"
    f"{' | SNAP=ON' if SNAP_MODE else ''}"
)
