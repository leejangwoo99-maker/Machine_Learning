...
def _manual_mail_log_path() -> Path:
    try:
        base = Path(r"C:\AI_dashboard_rev7")
        p = base / "logs"
        p.mkdir(parents=True, exist_ok=True)
        return p / "manual_stop_mail.log"
    except Exception:
        return Path(r"C:\AI_dashboard_rev7\logs\manual_stop_mail.log")


def _manual_mail_log(msg: str) -> None:
    line = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {msg}"
    try:
        with open(_manual_mail_log_path(), "a", encoding="utf-8") as f:
            f.write(line + "\n")
    except Exception:
        pass


def _resolve_run_mailer_command_for_manual() -> List[str]:
    candidates: List[Path] = []

    env_path = str(os.getenv("RUN_MAILER_EXE", "") or os.getenv("SNAP_RUN_MAILER_EXE", "")).strip()
    if env_path:
        candidates.append(Path(env_path))

    aptiv_root = str(os.getenv("APTIV_ROOT", "")).strip()
    if aptiv_root:
        root = Path(aptiv_root)
        candidates.extend(
            [
    ...
    try:
        ...
        candidates.extend(
            [
                here.parents[3] / "run_mailer.dist" / "run_mailer.exe",
                here.parents[4] / "run_mailer.dist" / "run_mailer.exe",
                here.parents[5] / "run_mailer.dist" / "run_mailer.exe",
                here.parents[3] / "launcher.dist" / "run_mailer.exe",
                here.parents[4] / "launcher.dist" / "run_mailer.exe",
            ]
        )
    except Exception:
        pass
    ...
    _manual_mail_log("resolve candidates:")
    for p in uniq:
        _manual_mail_log(f"  - {p}")

    for p in uniq:
        try:
            if p.exists() and p.is_file() and p.name.lower() == "run_mailer.exe":
                _manual_mail_log(f"resolved run_mailer.exe = {p}")
                return [str(p), "--manual-stop-mail"]
        except Exception:
            continue

    checked = "\n".join(str(p) for p in uniq) if uniq else "(none)"
    _manual_mail_log(f"run_mailer.exe not found. checked=\n{checked}")
    raise FileNotFoundError(f"run_mailer.exe not found. checked:\n{checked}")
...
def _run_production_abnormal_stop() -> Dict[str, Any]:
    ...
    try:
        cmd = _resolve_run_mailer_command_for_manual()
        exe_path = Path(cmd[0]).resolve()
        run_cwd = str(exe_path.parent)

        env = os.environ.copy()
        env["RUN_MAILER_EXE"] = str(exe_path)
        env["APTIV_ROOT"] = r"C:\AI_dashboard_rev7"

        _manual_mail_log(f"START manual stop cmd={cmd}")
        _manual_mail_log(f"START manual stop cwd={run_cwd}")
        _manual_mail_log(f"START manual stop APTIV_ROOT={env.get('APTIV_ROOT', '')}")
        ...
        stdout_txt = str(proc.stdout or "")[-20000:]
        stderr_txt = str(proc.stderr or "")[-20000:]

        _manual_mail_log(f"END rc={proc.returncode} elapsed_ms={ms:.0f}")
        if stdout_txt.strip():
            _manual_mail_log(f"STDOUT:\n{stdout_txt}")
        if stderr_txt.strip():
            _manual_mail_log(f"STDERR:\n{stderr_txt}")
        ...
        return {
            ...
            "stdout": stdout_txt,
            "stderr": stderr_txt,
            "error": "" if ok else f"run_mailer rc={proc.returncode}",
            "trace": "",
        }

    except subprocess.TimeoutExpired as e:
        ms = (time_mod.perf_counter() - t0) * 1000.0
        _manual_mail_log(f"TIMEOUT cmd={getattr(e, 'cmd', None)}")
        return {
            "ok": False,
            "ms": ms,
            "returncode": -9,
            "cmd": getattr(e, "cmd", None),
            "stdout": str(getattr(e, "stdout", "") or "")[-20000:],
            "stderr": str(getattr(e, "stderr", "") or "")[-20000:],
            "error": "run_mailer timeout",
        ...
        _manual_mail_log(f"EXCEPTION {e}\n{tb}")
        return {
            "ok": False,
            "ms": ms,
            "returncode": -1,
...
def _mount_alarm_sse(cur_day: str, cur_shift: str):
    ...
    (function () {
      ...
      function ackAndRerun(doc, pk) {
        try {
          addAck(pk);

          const u = new URL(W.location.href);
          u.searchParams.set("ack_alarm_pk", pk);
        ...
        try {
          W.postMessage({isStreamlitMessage: true, type: "streamlit:rerunScript"}, "*");
        } catch (e) {}
      }
      ...
      function handleAlarm(obj, fromEvent) {
        ...
        if (getAcked().includes(pk)) {
          log("already acked pk=", pk);
          return;
        }
    ...
    (function() {{
      ...
      box.innerHTML = `
        ...
        <div style="margin-top:16px; display:flex; gap:12px;">
          ...
          <button id="alarmRerunBtn_${{pk}}" style="flex:1; padding:10px 0; border-radius:10px;
                  border:1px solid #ddd; background:white; cursor:pointer; font-size:15px;">
            새로고침
          </button>
        </div>
      ...
      const xBtn = doc.getElementById("alarmCloseX_" + pk);
      const aBtn = doc.getElementById("alarmAckBtn_" + pk);
      const rBtn = doc.getElementById("alarmRerunBtn_" + pk);
...
def _clear_pending_alarm_state():
    st.session_state["pending_alarm_pk"] = ""
    st.session_state["pending_alarm_msg"] = ""
    st.session_state.pop("pending_alarm_pk", None)
    st.session_state.pop("pending_alarm_msg", None)
    st.session_state["pending_alarm_pk"] = ""
    st.session_state["pending_alarm_msg"] = ""


def _consume_ack_alarm_param() -> str:
    ack_pk = _qparam_get("ack_alarm_pk", "").strip()
    if not ack_pk:
        return ""

    try:
        st.session_state.seen_alarm_pks.add(ack_pk)
    except Exception:
        pass

    try:
        pending_pk = str(st.session_state.get("pending_alarm_pk", "") or "").strip()
        if pending_pk == ack_pk:
            _clear_pending_alarm_state()
        else:
            _clear_pending_alarm_state()
    except Exception:
        _clear_pending_alarm_state()
    ...
    try:
        ...
        try:
            st.query_params.pop("ack_alarm_pk")
        except Exception:
            pass
...
def frag_sync_data(end_day: str, shift: str):
    ...
    try:
        ack_pk = _consume_ack_alarm_param()
        if ack_pk:
            _perf_push("alarm_ack_consumed_sync", 0.0, {"ack_alarm_pk": ack_pk})
    except Exception:
        pass
    ...
    if alarm_tok and (not alarm_tok.startswith("__ERR__")):
        try:
            ...
            ack_pk = _qparam_get("ack_alarm_pk", "").strip()
            if ack_pk and ack_pk == pk:
                st.session_state.seen_alarm_pks.add(pk)
                _clear_pending_alarm_state()
                try:
                    st.query_params.pop("ack_alarm_pk", None)
                except Exception:
...
@_frag_deco(None)
def frag_alarm_once():
    pk = str(st.session_state.get("pending_alarm_pk", "") or "").strip()
    msg = str(st.session_state.get("pending_alarm_msg", "") or "").strip()
        ...
        if st.button("생산 비정상 STOP", use_container_width=True, key=SNAPSHOT_BTN_KEY):
            ...
            if ok2:
                ...
                with st.expander("에러 상세(log)", expanded=False):
                    ...
                    if str(res.get("stdout", "") or "").strip():
                        st.code(str(res.get("stdout", ""))[-8000:], language="text")
                    if stderr_txt:
...
def _render_snap_ready_marker(end_day: str, shift: str):
    ...
    info = _snap_data_ready_state(end_day, shift)
    ready = bool(info.get("ready", False))

    attrs = {
        "id": "snap-ready",
        "data-snap-ready": "1" if ready else "0",
        "data-planned-ready": "1" if info.get("planned_ready") else "0",
        "data-nonop-ready": "1" if info.get("nonop_ready") else "0",
        "data-nonop-chart-ready": "1" if info.get("nonop_chart_ready") else "0",
        "data-worker-ready": "1" if info.get("worker_ready") else "0",
        "data-master-ready": "1" if info.get("master_ready") else "0",
...
def _snap_mark_ready():
    ...
    components.html(
        ...
        (function () {
          ...
          function removeOld() {
            const old = D.getElementById("__snap_ready__");
            if (old) old.remove();
          }
...
if not SNAP:
    try:
        ack_pk = _consume_ack_alarm_param()
        if ack_pk:
            _perf_push("alarm_ack_consumed_main", 0.0, {"ack_alarm_pk": ack_pk})
    except Exception:
        pass
