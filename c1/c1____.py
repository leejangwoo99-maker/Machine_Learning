...
def parse_filename(filepath: Path):
    name = filepath.stem
    barcode = name.split("_", 1)[0] if "_" in name else name
...
def parse_one_file_to_rows(file_path: Path):
    ...
    end_time = get_end_time_str(events[-1][0])
...
def ensure_schema_and_tables(conn):
    """
    - 결과 스키마/테이블 생성 + 컬럼 구조 보정만 수행
    - processing 이력 테이블은 생성/사용하지 않음
    - group 번호는 시퀀스(group_seq)로 멀티프로세스에서도 안전하게 발급
    - row 중복 방지는 (file_path, test_time, test_item) UNIQUE로 처리
    """
    with conn.cursor() as cur:
        ...
        cur.execute(
            f"""
            CREATE SEQUENCE IF NOT EXISTS {SCHEMA_RESULT}.group_seq
            START 1;
            """
        )

        # yyyymmdd -> end_day 컬럼명 변경 (있을 때만)
        cur.execute(
            f"""
            DO $$
            BEGIN
        ...
        # ✅ row 단위 중복 방지 인덱스 (DO 블록 없이 깔끔하게)
        cur.execute(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS fct_testlog_detail_result_row_uq
            ON {SCHEMA_RESULT}.{TABLE_RESULT} (file_path, test_time, test_item);
            """
        )
...
def get_processed_file_paths(conn):
    """
    결과 테이블에서 DISTINCT file_path 로 파일 단위 재처리 방지.
    """
    with conn.cursor() as cur:
        cur.execute(
            f"""
...
# 3. 파일 단위 즉시 후처리
# =====================================================================
def postprocess_for_one_file(cur, file_path_str: str):
    # 1) Vision JSON에서 station, result 매핑 (해당 file_path 행만)
    cur.execute(
        f"""
    ...
    # 2) remark = PD / Non-PD (해당 file_path 행만)
    cur.execute(
        f"""
    ...
    # 3) group: 시퀀스로 안전하게 발급 (파일 단위 1개 group)
    cur.execute(
        f"""
        WITH g AS (
            SELECT nextval('{SCHEMA_RESULT}.group_seq') AS grp
        )
        UPDATE {SCHEMA_RESULT}.{TABLE_RESULT} t
        SET "group" = (SELECT grp FROM g)
        WHERE t.file_path = %s
          AND t."group" IS NULL;
        """,
        (file_path_str,),
    ...
    # 4) problem1/2/3, fail_test_item 초기화 (해당 file_path 행만)
    cur.execute(
        f"""
        UPDATE {SCHEMA_RESULT}.{TABLE_RESULT}
        SET problem1 = NULL,
    ...
    # 6) Non-PD 매핑 (해당 file_path 행만)  ← 기존 블록 그대로 사용
    cur.execute(
        f"""
        WITH candidates AS (
            SELECT
...
def insert_and_postprocess_one_file(file_path_str: str, rows: list[dict]):
    if not rows:
        return 0
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # 결과 INSERT (row_uq 인덱스가 충돌 대상이 됨)
            cols = (
                "file_path",
                "end_day",
                "end_time",
            ...
            # 즉시 후처리(파일 범위)
            postprocess_for_one_file(cur, file_path_str)
...
def process_and_insert_one_file(file_path_str: str):
    try:
        ...
        inserted = insert_and_postprocess_one_file(file_path_str, rows)
...
def run_once():
    ...
    print("5) 멀티프로세스(파싱+DB저장+즉시후처리) 시작...")
