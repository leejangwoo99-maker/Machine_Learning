# -*- coding: utf-8 -*-
"""
backend9_planned_and_nonop_daemon_TEST_FIXED_20260128_DAY.py

[테스트 고정 버전]
- prod_day  = 20260128 (고정)
- shift     = day (고정)
- window    = 2026-01-28 08:30:00 ~ now(KST) (단, day shift max end=20:29:59로 clamp)

요구사항 반영:
1) dataframe 콘솔 출력 제외 (로그만)
2) 날짜는 [WINDOW]기준 현재(KST) 날짜/시각으로 자동 전환 (window_end=now)
   -> ✅ 테스트 고정: prod_day/shift는 고정, end(now)만 현재 시각
3) 멀티프로세스 1개
4) 무한루프 5초
...
def fixed_window_state(now: datetime) -> WindowState:
    """
    ✅ 테스트 고정:
    - prod_day/shift_type 고정
    - start_ts는 shift 고정 시작
    - end_ts는 now (단 shift max end로 clamp는 offset 계산에서 수행)
    """
    d0 = parse_yyyymmdd(FIXED_PROD_DAY)
    if FIXED_SHIFT_TYPE == "day":
        start_ts = datetime(d0.year, d0.month, d0.day, 8, 30, 0, tzinfo=KST)
    else:
        start_ts = datetime(d0.year, d0.month, d0.day, 20, 30, 0, tzinfo=KST)
    return WindowState(FIXED_PROD_DAY, FIXED_SHIFT_TYPE, start_ts, now)
...
# =========================
def main():
    log("BOOT", f"backend9 TEST starting (prod_day={FIXED_PROD_DAY}, shift={FIXED_SHIFT_TYPE})")
    ...
    # 고정 윈도우 로그 1회
    now0 = datetime.now(KST).replace(microsecond=0)
    ws0 = fixed_window_state(now0)
    log("INFO", f"[WINDOW][FIXED] prod_day={ws0.prod_day} shift={ws0.shift_type} start={ws0.start_ts.isoformat()} end(now)={ws0.end_ts.isoformat()}")

    while True:
        try:
            now = datetime.now(KST).replace(microsecond=0)
            ws = fixed_window_state(now)

            # planned_time: 매 루프 재조회
            planned_df = planned_fetch_for_window(engine, ws.prod_day)

            # bootstrap(최초 1회)
            if last_pk is None or nonop_rows_df is None:
                log("INFO", "[BOOTSTRAP] start (load all nonop rows for prod_day+1 dates)")
                all_df = fct_nonop_bootstrap(engine, ws.prod_day)
            ...
            inc_df = fct_nonop_incremental(engine, ws.prod_day, last_pk)
            ...
            # 계산
            df_planned = planned_rows_per_interval(planned_df, ws.prod_day, ws.shift_type, now, include_total_row=True)
            df_nonop = nonop_summary_overlap_spec_from_rows(nonop_rows_df, planned_df, ws.prod_day, ws.shift_type, now)
            ...
            # UPSERT (shift 고정이므로 day로만 저장)
            KEY_PLAN = ["prod_day", "shift_type", "from_time", "to_time"]
            KEY_NONOP = ["prod_day"]

            if ws.shift_type == "day":
