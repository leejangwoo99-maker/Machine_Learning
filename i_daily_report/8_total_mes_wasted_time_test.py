# -*- coding: utf-8 -*-
"""
backend8_total_mes_loss_time_daemon_TEST_20260128_DAY.py

[TEST 고정 버전: 20260128 day]
...
13) 재실행 시 DELETE/TRUNCATE 금지
    - last_pk가 날아가므로 현재 윈도우(start~end) bootstrap 후 UPSERT

[반영된 수정]
- fetch_increment_mes_fail(): (from,to]가 현재 윈도우(start~end)와 "겹치지 않으면" 완전 skip
...
TEST_SHIFT_TYPE = "day"  # 고정
TEST_START = datetime(2026, 1, 28, 8, 30, 0, tzinfo=KST)
TEST_END = datetime(2026, 1, 28, 20, 29, 59, tzinfo=KST)
...
def fixed_window() -> Window:
    """
    테스트 고정 윈도우:
    - prod_day=20260128
    - day: 08:30:00 ~ 20:29:59
    """
    pd_day = datetime.strptime(TEST_PROD_DAY, "%Y%m%d").date()
    ...
    # 고정 end는 TEST_END 사용
    return Window(TEST_PROD_DAY, TEST_SHIFT_TYPE, TEST_START, TEST_END, day_start, day_end, night_start, night_end)
...
# =========================
# 5) mes_fail_wasted_time split (day/night)
# =========================
...
def fetch_increment_mes_fail(engine: Engine, w: Window, state: State) -> int:
    ...
    for r in df.itertuples(index=False):
        ...
        # (from,to] 가 고정 윈도우와 완전 미겹치면 skip
        ...
        day_w, _, day_c, _ = split_row_day_night(
            end_day=str(r.end_day),
            from_time_txt=str(r.from_time),
            to_time_txt=str(r.to_time),
            wasted_time_db=r.wasted_time,
...
# =========================
def main():
    log_boot("backend8 total MES loss-time daemon [TEST FIXED 20260128 DAY] starting")
    ...
    log_info(f"[TEST_WINDOW] prod_day={w.prod_day} shift={w.shift_type} start={w.start} end={w.end}")
    ...
    while True:
        try:
            ...
            # 재연결 직후 bootstrap+upsert
            try:
                log_info(f"[RECOVER] bootstrap after reconnect prod_day={w.prod_day} shift={w.shift_type}")
                ...
                upsert_daily(engine, target_table(w.shift_type), row)
                log_info(
                    f"[UPSERT] recover {SAVE_SCHEMA}.{target_table(w.shift_type)} prod_day={row['prod_day']} "
                    f"shift={w.shift_type} total={row['Total MES 불량 손실 시간(초)']}"
                )
            except Exception as ee:
