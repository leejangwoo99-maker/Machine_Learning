...
# =========================
# Scheduler Helpers (NEW)
# =========================
def _in_window(now_dt: datetime, start_t: dt_time, end_t: dt_time) -> bool:
    t = now_dt.time()
    return (t >= start_t) and (t <= end_t)


def _next_window_start(now_dt: datetime, windows):
    today = now_dt.date()

    candidates = []
    for s, _e in windows:
        cand = datetime.combine(today, s)
        if cand > now_dt:
    ...
    return datetime.combine(tomorrow, first_start)


if __name__ == "__main__":
    # 2 daily windows
    WINDOWS = [
        (dt_time(8, 27, 0), dt_time(8, 29, 59)),
        (dt_time(20, 27, 0), dt_time(20, 29, 59)),
    ]

    print("[INFO] Scheduler mode enabled")
    print("[INFO] Windows: 08:27:00~08:29:59, 20:27:00~20:29:59")
    print("[INFO] Loop: run main() every 1 second within windows")

    while True:
        now = datetime.now()

        if any(_in_window(now, s, e) for s, e in WINDOWS):
            # within window: run main() every 1 sec
            _start_ts = time.time()
            ...
            try:
                ...
                # keep looping even on error during window time
                print("[ERROR]", repr(e))
            finally:
                _end_ts = time.time()
            ...
            # keep 1-second tick (best-effort)
            sleep_sec = max(0.0, 1.0 - (time.time() - _start_ts))
            time.sleep(sleep_sec)

        else:
            # outside window: sleep until next window (up to 60s chunks)
            nxt = _next_window_start(now, WINDOWS)
            wait_sec = max(1.0, (nxt - now).total_seconds())
            print(f"[INFO] Outside window. Next run window starts at: {nxt:%Y-%m-%d %H:%M:%S}")
