...
# ★ 분석 기간 고정: 2025-10-01 ~ 2025-11-30 (VIEW end_day 타입이 'YYYYMMDD' text라고 가정)
START_YMD = "20251001"
END_YMD   = "20251130"

print("=== [1] DB 엔진 생성 완료 ===")
print("Connection String:", conn_str)
print(f"분석 기간: {START_YMD} ~ {END_YMD}")
print()
...
def process_once():
    ...
    query_with_join = text(f"""
        ...
        WHERE v.result <> 'FAIL'
          AND v.end_day >= :start_ymd          -- ★ 기간 필터
          AND v.end_day <= :end_ymd            -- ★ 기간 필터
    ...
    query_without_join = text(f"""
        ...
        FROM {VIEW_NAME}
        WHERE result <> 'FAIL'
          AND end_day >= :start_ymd            -- ★ 기간 필터
          AND end_day <= :end_ymd              -- ★ 기간 필터
    """)
    ...
    try:
        with engine.connect() as conn:
            df_raw = pd.read_sql(
                query_with_join,
                conn,
                params={"start_ymd": START_YMD, "end_ymd": END_YMD},  # ★
            )
