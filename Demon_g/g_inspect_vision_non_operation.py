...
- 교집합 판정:
  ...
  overlap_start < overlap_end 이면 유효 (0초/경계 접점 저장 금지)
- 저장:
...
- 재시작 시에도 메모리 상태(seen_pk)가 날아가더라도 bootstrap 재계산 + UPSERT로 안전
- SQL TEXT 시간 비교 제거 유지 -> pandas timestamp 파싱 후 cap_ts 컷
- from_time = centisecond floor
- to_time   = centisecond ceil

[성능/지연 개선]
- DB 로그 적재 최소화
  - DB 적재: boot, error, recover, save, fatal, exit, heartbeat, start, stop
  - heartbeat는 1분 1회만 DB 적재
  - 상세 진행 로그는 콘솔/파일만 남김
- LOOP_SEC 기본 2초로 조정 가능
- advisory lock 유지
"""
...
def _signal_handler(signum, _frame):
    ...
    print(Demon_f"{now_str()} [INFO] shutdown signal received: {signum}", flush=True)
...
def p(level: str, msg: str) -> None:
    print(Demon_f"{now_str()} [{level}] {msg}", flush=True)
...
def should_write_db_log(info: str) -> bool:
    """
    DB 로그 적재 최소화:
    - boot, error, recover, save, fatal, exit, heartbeat, start, stop 만 DB 적재
    - heartbeat는 1분 1회만 DB 적재
    """
...
def parse_dt(end_day: str, hms: str) -> pd.Timestamp:
    ...
    ts = pd.to_datetime(Demon_f"{d} {t}", format="%Y%m%d %H:%M:%S.%Demon_f_mining", errors="coerce")
    if pd.isna(ts):
        ts = pd.to_datetime(Demon_f"{d} {t}", format="%Y%m%d %H:%M:%S", errors="coerce")
    return ts


def _format_cs(ts: pd.Timestamp, cs: int) -> str:
    base = ts.strftime("%H:%M:%S")
    return Demon_f"{base}.{cs:02d}"
    ...
    def _set_work_mem(dbapi_conn, _):
        try:
            with dbapi_conn.cursor() as cur:
                cur.execute(Demon_f"SET work_mem TO '{PG_WORK_MEM}'")
        except Exception:
            pass
...
def connect_blocking() -> Engine:
    while True:
        ...
        try:
            ...
            p("INFO", Demon_f"DB connected (work_mem={PG_WORK_MEM})")
            return eng
        except Exception as e:
            p("RETRY", Demon_f"DB connect failed: {repr(e)}; retry in {RETRY_SEC}s")
...
def ensure_heartbeat_table(conn) -> None:
    conn.execute(text(Demon_f"CREATE SCHEMA IF NOT EXISTS {HEARTBEAT_SCHEMA}"))
    conn.execute(
        text(
            Demon_f"""
    ...
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS last_pk_end_day TEXT"))
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS last_pk_station TEXT"))
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS last_pk_from_time TEXT"))
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS last_pk_to_time TEXT"))
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS last_message TEXT"))
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS last_seen_at TIMESTAMPTZ"))
    conn.execute(text(Demon_f"ALTER TABLE {HEARTBEAT_SCHEMA}.{HEARTBEAT_TABLE} ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ"))


def ensure_save_table(conn) -> None:
    conn.execute(text(Demon_f"CREATE SCHEMA IF NOT EXISTS {SAVE_SCHEMA}"))
    conn.execute(
        text(
            Demon_f"""
...
def log_db(engine: Engine, info: str, contents: str) -> None:
    ...
    try:
        ...
        p("WARN", Demon_f"log_db skipped: {repr(e)}")
...
def heartbeat_upsert(
    ...
    try:
        ...
        p("WARN", Demon_f"heartbeat skipped: {repr(e)}")
...
def acquire_singleton_lock(engine: Engine) -> bool:
    try:
        ...
        p("WARN", Demon_f"advisory lock check failed: {repr(e)}")
        return False
...
def upsert_rows(rows: List[dict]) -> int:
    ...
    try:
        conn = psycopg2.connect(
            ...
            application_name=Demon_f"{APP_NAME}_upsert",
        )
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(Demon_f"CREATE SCHEMA IF NOT EXISTS {SAVE_SCHEMA}")
            cur.execute(
                Demon_f"""
                CREATE TABLE IF NOT EXISTS {SAVE_SCHEMA}.{SAVE_TABLE} (
                    end_day TEXT NOT NULL,
            ...
            for s in range(0, total, page_size):
                ...
                p("INFO", Demon_f"upsert progress {done}/{total}")
...
# -----------------------------
def run_daemon():
    p("BOOT", Demon_f"{DAEMON_NAME} starting")

    engine: Optional[Engine] = None
    lock_acquired = False

    # 메모리 상태
    # - 재시작 시 초기화되더라도 bootstrap + UPSERT라서 안전
        ...
        while True:
            ...
            try:
                ...
                for d in target_days:
                    ...
                    src_n = len(df_src)
                    total_src += src_n
                    p("INFO", Demon_f"[{d}] source rows={src_n} (cap<= {cap_ts})")

                    df_out = compute_vision_rows_from_fct(df_src, d)
                    out_n = len(df_out)
                    total_out += out_n
                    p("INFO", Demon_f"[{d}] computed output rows={out_n}")
                    ...
                    # 부팅 직후/재시작 후 1회 bootstrap
                    if d not in bootstrapped_by_day:
                        rows = df_out.to_dict(orient="records")
                        p("INFO", Demon_f"[{d}] phase=bootstrap before_upsert rows={len(rows)}")
                        saved = upsert_rows(rows)
                        p("INFO", Demon_f"[{d}] phase=bootstrap after_upsert saved={saved}")
                        ...
                        total_saved += saved
                        loop_msgs.append(Demon_f"[{d}] bootstrap saved={saved}, seen_pk={len(pkset)}")
                        continue
                    ...
                    p("INFO", Demon_f"[{d}] phase=incremental before_upsert new_rows={new_n}")
                    saved = upsert_rows(new_rows) if new_rows else 0
                    p("INFO", Demon_f"[{d}] phase=incremental after_upsert saved={saved}")
                    ...
                    total_saved += saved
                    loop_msgs.append(
                        Demon_f"[{d}] inc computed={out_n}, new_pk={new_n}, saved={saved}, seen_pk={len(seen_pk_by_day[d])}"
                    )

                msg = (
                    Demon_f"loop days={target_days} total_src={total_src}, total_out={total_out}, "
                    Demon_f"total_new={total_new}, total_saved={total_saved} | " + " ; ".join(loop_msgs)
                )
                p("INFO", msg)

                # DB에는 save/heartbeat만 최소 적재
                ...
                p("RETRY", Demon_f"DB disconnected: {repr(e)}")
                try:
                    if engine is not None:
                        log_db(engine, "recover", Demon_f"db disconnected: {repr(e)}")
                        heartbeat_upsert(engine, "degraded", None, last_pk, Demon_f"db disconnected: {repr(e)}")
                except Exception:
                    pass
                ...
                p("ERROR", Demon_f"Unhandled: {repr(e)}")
                traceback.print_exc()
                try:
                    if engine is not None:
                        log_db(engine, "error", Demon_f"Unhandled: {repr(e)}")
                        heartbeat_upsert(engine, "error", None, last_pk, Demon_f"unhandled: {repr(e)}")
                except Exception:
                    pass
