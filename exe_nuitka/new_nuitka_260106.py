...
#    (주의) 이 venv에 xgboost가 설치되어 있어야 합니다.
# ---------------------------------------------------------
...
def run_compile(disable_console: bool = False):
    ...
    # ---------------------------------------------------------
    # XGBoost DLL 보험 포함 (특히 onefile에서 런타임 크래시 방지)
    # ---------------------------------------------------------
    xgb_dlls = get_xgboost_dll_candidates()
    for dll_path in xgb_dlls:
        # 목적지 경로는 패키지 내부 구조 유지 쪽이 안전 (xgboost/...)
        # Nuitka 옵션 형식: --include-data-files=SRC=DEST
    ...
    if xgb_dlls:
        ...
        txt_log.insert(tk.END, "XGBoost DLL 감지: 0개 (환경에 따라 정상일 수도 있으나, 런타임 크래시 시 이 부분 점검)\n")
...
txt_log.insert(tk.END, "[INFO] XGBoost 포함 옵션 및 DLL 보험 포함 로직이 활성화되었습니다.\n\n")
txt_log.see(tk.END)
