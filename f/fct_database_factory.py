    ...
    while True:
        ...
        try:
            ...
            with raw.cursor() as cur:
                cur.execute(f"CREATE UNLOGGED TABLE IF NOT EXISTS {staging} (LIKE {OUT_SCHEMA}.{OUT_TABLE} INCLUDING DEFAULTS);")
                raw.commit()
...
def main():
    ...
    while cur <= d_to:
        ...
        log_event("info", f"main day done day={cur} rows_final={len(df_final)} sec={time.time()-day_start:.2f}")
        cur = cur + timedelta(days=1)
...
def realtime_loop_single():
    ...
    bootstrap_log_table(None)
    log_event("info", f"realtime start loop={REALTIME_LOOP_INTERVAL_SEC}s limit={REALTIME_FETCH_LIMIT_ROWS} lookback={REALTIME_LOOKBACK_SEC}s work_mem={WORK_MEM_MB}MB")
    ...
    while True:
        ...
        if last_day is None or day_ != last_day:
            try:
                ...
                pass
            last_day = day_
            log_event("info", f"day rollover detected day={day_}")
